...
// Copyright(c) 2023 AllVerge.net / Paul Wilcox
...
namespace AllVerge.MessagingModel.MessagingFoundation.Description
{
    ...
    internal static class ResourceActionMethodAttributeExtensions
    {
        public static bool AsyncPattern(this ResourceEndpointAttribute resourceMethodAttribute)
        {
            return (resourceMethodAttribute as MessageActionAttribute).AsyncPattern;
        }

        public static bool IsOneWay(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)

                return (opSettings as MessageActionAttribute).IsOneWay();

            if (opSettings is WaitForReplyMessageAsyncMethodAttribute)

                return (opSettings as WaitForReplyMessageAsyncMethodAttribute).IsOneWay();

            return false;
        }

        public static bool IsOneWay(this MessageActionAttribute opSettings)
        {
            if (opSettings != null)
                return opSettings.GetIsOneWay();
            return false;
        }

        public static bool IsOneWay(this WaitForReplyMessageAsyncMethodAttribute opSettings)
        {
            // Since GetResponseMethodForCallbackAttribute is used to decorate a method with which to obtain a duplex callback response, 
            // essetially acting as a bridge between duplex input and output methods; then we treat this as a non-one-way interaction ...
            if (opSettings != null)
                return false;
            return false;
        }
        ...
        public static String ReplyAction(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)
            {
                return (opSettings as MessageActionAttribute).ReplyAction;
            }

            return null;
        }

        public static bool IsInitiating(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)

                return (opSettings as MessageActionAttribute).IsInitiating();

            if (opSettings is WaitForReplyMessageAsyncMethodAttribute)

                return (opSettings as WaitForReplyMessageAsyncMethodAttribute).IsInitiating();

            return true;
        }

        public static bool IsInitiating(this MessageActionAttribute opSettings)
        {
            if (opSettings != null)
                return opSettings.IsInitiating; // by default this will be true ...
            return false;
        }

        public static bool IsInitiating(this WaitForReplyMessageAsyncMethodAttribute opSettings)
        {
            //Since by default, resource actions are initiating, and this attribute is to obtain the value used to initiate 
            // a (possible) session, we return true here ...
            if (opSettings != null)
                return true;
            return false;
        }

        public static bool IsTerminating(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)

                return (opSettings as MessageActionAttribute).IsTerminating();

            return false;
        }

        public static bool IsTerminating(this MessageActionAttribute opSettings)
        {
            if (opSettings != null)
                return opSettings.IsTerminating;
            return false;
        }

        public static bool IsSessionOpenNotificationEnabled(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)

                return (opSettings as MessageActionAttribute).IsSessionOpenNotificationEnabled();

            return false;
        }

        public static bool IsSessionOpenNotificationEnabled(this MessageActionAttribute opSettings)
        {
            if (opSettings != null)
                return opSettings.IsSessionOpenNotificationEnabled;
            return false;
        }

        public static bool HasProtectionLevel(this ResourceEndpointAttribute opSettings)
        {
            if (opSettings is MessageActionAttribute)

                return (opSettings as MessageActionAttribute).HasProtectionLevel();

            return false;
        }

        public static bool HasProtectionLevel(this MessageActionAttribute opSettings)
        {
            if (opSettings != null)
                return opSettings.HasProtectionLevel;
            return false;
        }

        public static bool IsBegin(this ResourceEndpointAttribute opSettings, MethodInfo method)
        {
            if (opSettings != null)
            {
                if (opSettings.AsyncPattern)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.InvalidAsyncBeginMethodSignatureForMethod2, method.Name, method.DeclaringType.FullName)));
                    }
        ...
        public static void EnsureInvariants(this ResourceEndpointAttribute opSettings, MethodInfo method, string operationName)
        {
            if (opSettings is MessageActionAttribute)

                (opSettings as MessageActionAttribute).EnsureInvariants(method, operationName);
        }

        public static void EnsureInvariants(this MessageActionAttribute opSettings, MethodInfo methodInfo, string operationName)
        {
            if (opSettings != null)
            {
                if (opSettings.IsSessionOpenNotificationEnabled)
                {
                    if (!opSettings.GetIsOneWay()
                     || !opSettings.IsInitiating
                     || methodInfo.GetParameters().Length > 0)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            PublicSR.Format(PublicSR.ContractIsNotSelfConsistentWhenIsSessionOpenNotificationEnabled, operationName, "Action", OperationDescription.SessionOpenedAction, "IsOneWay", "IsInitiating")));
                    }
                }
            }
        }
    ...
    internal class ResourceTypeLoader: ITypeLoader
    {
        ...
        private static Type[] s_formatterAttributes = {
            typeof(XmlAttributeOverridesSerializerFormatAttribute),
        ...
        private static readonly Type s_OperationContractAttributeType = typeof(ResourceActionAttribute);

        static ResourceTypeLoader()
        {
            DataContractSerializationHelper.RegisterContractAttributeType(typeof(MessageContractAttribute));
        }
        ...
        private readonly bool _channelFactory;

        public ResourceTypeLoader(bool channelFactory = false)
        {
        ...
        public bool ExpectDuplexCallbackOnContractType => true;
        ...
        private ContractDescription LoadContractDescriptionHelper(Type contractType, Type serviceType, object serviceImplementation)
        {
            ...
            {
                Type actualContractType = ServiceReflector.GetContractTypeAndAttribute(contractType, out ResourceContractAttribute actualContractAttribute);
                lock (_thisLock)
                {
                    if (!_contracts.TryGetValue(actualContractType, out contractDescription))
                    {
                        ...
                        contractDescription = CreateContractDescription(actualContractAttribute, actualContractType, serviceType, out ContractReflectionInfo reflectionInfo, serviceImplementation);
                        // IContractBehaviors
                        ...
                        if (serviceType != null)
                        {
                            UpdateOperationsWithKnownTypesFromServiceType(contractDescription.Operations, serviceType);
                        ...
                        AddBehaviors(contractDescription, serviceType, reflectionInfo);
        ...
        private void UpdateOperationsWithKnownTypesFromServiceType(OperationDescriptionCollection operations, Type serviceType)
        {
            while (serviceType != null)
            {
                object[] customAttributes = ServiceReflector.GetCustomAttributes(serviceType, typeof(ServiceKnownTypeAttribute), false);
                IEnumerable<Type> knownTypes = GetKnownTypes(customAttributes, serviceType);
        ...
        private void EnsureNoInheritanceWithContractClasses(Type actualContractType)
        {
            if (actualContractType.IsClass)
            {
                ...
                {
                    if (ServiceReflector.GetSingleAttribute<ResourceContractAttribute>(service) != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            PublicSR.Format(PublicSR.SFxContractInheritanceRequiresInterfaces, actualContractType, service)));
                    }
                }
            }
        }
        ...
        private void EnsureNoOperationContractsOnNonServiceContractTypes_Helper(Type aParentType)
        {
            ...
            if (ServiceReflector.GetSingleAttribute<ResourceContractAttribute>(aParentType) == null)
            {
                foreach (MethodInfo methodInfo in aParentType.GetRuntimeMethods().Where(m => !m.IsStatic))
                {
                    // but does have an ResourceMethodAttribute
                    Type operationContractProviderType = ServiceReflector.GetOperationContractProviderType<ResourceActionAttribute, IResourceMethodAttributeProvider>(methodInfo);
                    if (operationContractProviderType != null)
                    {
                        if (operationContractProviderType == s_OperationContractAttributeType)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(
                                PublicSR.SFxOperationContractOnNonServiceContract, methodInfo.Name, aParentType.Name)));
                        }
                        else
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(
                                PublicSR.SFxOperationContractProviderOnNonServiceContract, operationContractProviderType.Name, methodInfo.Name, aParentType.Name)));
                        }
                    }
                }
            }
        ...
        private void AddBehaviors(ContractDescription contractDesc, Type implType, ContractReflectionInfo reflectionInfo)
        {
            ResourceContractAttribute contractAttr = ServiceReflector.GetRequiredSingleAttribute<ResourceContractAttribute>(reflectionInfo.iface);
            ...
            for (int i = 0; i < contractDesc.Operations.Count; i++)
            {
                ...
                AddOperationBehaviors(implType, opDesc.DeclaringContract.ContractType, opDesc, isInherited);
            }
            ...
            for (int i = 0; i < contractDesc.Operations.Count; i++)
            {
                ...
                Attribute formattingAttribute = GetInheritedSerializerFormatAttribute(implType, opMethod);
                if (formattingAttribute == null)
                {
                    formattingAttribute = GetSerializerFormatAttribute(opMethod, null);
                }
                if (formattingAttribute != null)
                {
                    if (!isInherited)
                    {
                        if (formattingAttribute is ResourceTransferFormatSerializerAttribute)
                        {
                            operationDescription.Behaviors.Add(new ResourceTransferFormatSerializerOperationBehavior(operationDescription, formattingAttribute as ResourceTransferFormatSerializerAttribute));
                        }
                        else if (formattingAttribute is XmlSerializerFormatAttribute)
                        {
                            operationDescription.Behaviors.Add(new XmlSerializerOperationBehavior(operationDescription, formattingAttribute as XmlSerializerFormatAttribute));
                        }
                        else if (formattingAttribute is XmlAttributeOverridesSerializerFormatAttribute)
                        {
                            operationDescription.Behaviors.Add(new XmlAttributeOverridesSerializerOperationBehavior(operationDescription, formattingAttribute as XmlAttributeOverridesSerializerFormatAttribute));
                        }
                        else if (formattingAttribute is DataContractFormatAttribute)
                        {
                            operationDescription.Behaviors.Add(new DataContractSerializerOperationBehavior(operationDescription, formattingAttribute as DataContractFormatAttribute, true));
                        }
                    }
                }
                else
                {
                    formattingAttribute = new ResourceTransferFormatSerializerAttribute();
        ...
        private string GetLogicalName(string methodName)
        {
            bool isTask = methodName.EndsWith(ServiceReflector.AsyncMethodNameSuffix, StringComparison.Ordinal);
            bool isAsync = methodName.StartsWith(ServiceReflector.BeginMethodNamePrefix, StringComparison.Ordinal);

            if (isAsync)
            {
                return methodName.Substring(ServiceReflector.BeginMethodNamePrefix.Length);
            }
            else if (isTask)
            {
                return methodName.Substring(0, methodName.Length - ServiceReflector.AsyncMethodNameSuffix.Length);
            }
            else
            {
                return methodName;
        ...
        private void AddOperationBehaviors(Type implType, Type targetIface, OperationDescription opDesc, bool isInherited)
        {
            if (implType == null && !isInherited)
            {
                ...
                {
                    if (toAdd[j] is ResourceActionAttribute)
                        continue;
        ...
        private void UpdateOperationsWithInterfaceAttributes(ContractDescription contractDesc, ContractReflectionInfo reflectionInfo)
        {
            ...
            foreach (OperationDescription duplexOperation in reflectionInfo.duplexCallbackTypesForInputOperations.Keys)
            {
                Type callbackContractType = reflectionInfo.duplexCallbackTypesForInputOperations[duplexOperation];
                customAttributes = ServiceReflector.GetCustomAttributes(callbackContractType, typeof(ServiceKnownTypeAttribute), false);
                if (duplexOperation.IsServerInitiated())
                {
                    knownTypes = GetKnownTypes(customAttributes, callbackContractType);
                    knownTypes.Aggregate(duplexOperation.KnownTypes, (c, t) => { c.Add(t); return c; });
                }
                foreach (Type knownType in knownTypes)
                {
                    foreach (OperationDescription operationDescription in contractDesc.Operations)
        ...
        private static IEnumerable<Type> GetKnownTypes(object[] knownTypeAttributes, ICustomAttributeProvider provider)
        {
            if (knownTypeAttributes.Length == 1)
            {
                ...
                if (!string.IsNullOrEmpty(knownTypeAttribute.MethodName))
                {
                    ...
                    if (method == null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxKnownTypeAttributeUnknownMethod3, provider, knownTypeAttribute.MethodName, type.FullName)));
                    }

                    if (!typeof(IEnumerable<Type>).IsAssignableFrom(method.ReturnType))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxKnownTypeAttributeReturnType3, provider, knownTypeAttribute.MethodName, type.FullName)));
                    }
            ...
            for (int i = 0; i < knownTypeAttributes.Length; ++i)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxKnownTypeAttributeInvalid1, provider.ToString())));
                }
                ...
                else
                {
                    if (opDesc.SyncMethod != null && opDesc.BeginMethod != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.SyncAsyncMatchConsistency_Attributes6,
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.SyncTaskMatchConsistency_Attributes6,
                                                                       opDesc.SyncMethod.Name,
                                                                       opDesc.SyncMethod.DeclaringType,
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.TaskAsyncMatchConsistency_Attributes6,
                                                                       opDesc.TaskMethod.Name,
        ...
        public void AddBehaviorsSFx(ServiceEndpoint serviceEndpoint, Type contractType)
        {
        ...
        public void AddBehaviorsFromImplementationType(ServiceEndpoint serviceEndpoint, Type implementationType)
        {
        ...
        )
        {
            ...
            foreach (MethodInfo methodInfo in contractType.GetRuntimeMethods().Where(m => !m.IsStatic))
            {
                ServiceReflector.ValidateParameterMetadata(methodInfo);
                IEnumerable<OperationDescription> operations = 
                    CreateOperationDescriptions(contractDescription, methodInfo, isCallbackContract, direction, reflectionInfo, declaringContract);

                foreach (OperationDescription operation in operations)
                {
                    contractDescription.Operations.Add(operation);

                    tryAddDuplexCallbackOperation(contractDescription, operation, reflectionInfo);
                }
            }

            IEnumerable<MessageActionAttribute> messageActionAttributes = contractDescription.Operations.SelectMany(o => o.Behaviors.Where(b => b is MessageActionAttribute).Select(bb => bb as MessageActionAttribute));

            // There should only be one message action attribute specified IsUnAddressedAction true (on any and all methods of the contract);

            if (messageActionAttributes.Count(a => a.IsUnAddressedAction) > 1)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidOperationException(
                    String.Format(CultureInfo.InvariantCulture, "Contract {0} has more than one method decorated with a MessageActionAttribute specifying IsDefaultAction true", contractType.FullName)
                    ));
            }

            // If there is no message action attribute (on any and all methods of the contract) specified IsUnAddressedAction true ...

            if (messageActionAttributes.All(a => !a.IsUnAddressedAction))
            {
                // And only one message action attribute (on any and all methods of the contract), then set IsUnAddressedAction = true

                if (messageActionAttributes.Count() == 1)
                {
                    messageActionAttributes.First().IsUnAddressedAction = true;
                }
            }

        }

        private bool tryAddDuplexCallbackOperation(ContractDescription contractDescription, OperationDescription possibleDuplexOperation, ContractReflectionInfo reflectionInfo)
        {
            PostMessageActionAttribute postMessageActionAttribute = 
                possibleDuplexOperation.Behaviors.Find<PostMessageActionAttribute>();

            if (postMessageActionAttribute != null && postMessageActionAttribute.GetIsDuplexResponse(out Type callbackResourceContractType))
            {
                string receiveMethodName = postMessageActionAttribute.Name ?? possibleDuplexOperation.Name;

                EnsureCallbackType(callbackResourceContractType);

                string logicalWaitForReplyAsyncMethod = null;

                if (this._channelFactory)
                {
                    if (possibleDuplexOperation.DeclaringContract.CallbackContractType == null)

                        possibleDuplexOperation.DeclaringContract.CallbackContractType = typeof(Object);
                }
                else
                {
                    MethodInfo waitForReplyAsyncMethod = contractDescription.ContractType.GetMethods().FirstOrDefault(m => m.GetCustomAttributes(typeof(WaitForReplyMessageAsyncMethodAttribute), true).Any(a => (a as WaitForReplyMessageAsyncMethodAttribute).ForReceivePostMethod == receiveMethodName));

                    if (waitForReplyAsyncMethod == null)

                        waitForReplyAsyncMethod = contractDescription.ContractType.GetMethods().FirstOrDefault(m => m.GetCustomAttributes(typeof(WaitForReplyMessageAsyncMethodAttribute), true).Any(a => (a as WaitForReplyMessageAsyncMethodAttribute).ForReceivePostMethod == null));

                    if (waitForReplyAsyncMethod == null)

                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new ArgumentException($"There is no method of the contract decorated with {nameof(WaitForReplyMessageAsyncMethodAttribute)} to identify the method with which to obtain a call back message to reply to a message received by ${receiveMethodName}."));

                    bool isTask = ServiceReflector.IsTask(waitForReplyAsyncMethod, out _);
                    bool isAsync = isTask ? false : ServiceReflector.HasBeginMethodShape(waitForReplyAsyncMethod);

                    logicalWaitForReplyAsyncMethod = ServiceReflector.GetLogicalName(waitForReplyAsyncMethod, isAsync, isTask);
                }

                CreateOperationDescriptions(contractDescription, reflectionInfo, callbackResourceContractType, true, contractDescription, MessageDirection.Output);

                String callbackContractMethodName = postMessageActionAttribute.CallbackContractMethodName;

                IEnumerable<OperationDescription> duplexOutputOperations = contractDescription.Operations.Where(o => o.DeclaringContract == contractDescription && o.IsOneWay && o.Messages.Count == 1 && o.Messages[0].Direction == MessageDirection.Output);

                OperationDescription callbackContractOperation;

                if (duplexOutputOperations.Count() > 1)
                {
                    if (String.IsNullOrEmpty(callbackContractMethodName))

                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException($"Cannot resolve the call back method of {callbackResourceContractType.FullName} to use.  Multiple options exist.  Please set {nameof(PostMessageActionAttribute.CallbackContractMethodName)} to a specific call back method."));

                    callbackContractOperation = duplexOutputOperations.FirstOrDefault(o => o.Name == callbackContractMethodName);
                }
                else
                {
                    callbackContractOperation = duplexOutputOperations.FirstOrDefault();

                    if (callbackContractOperation != null)
                    {
                        if (!String.IsNullOrEmpty(callbackContractMethodName) && callbackContractOperation.Name != callbackContractMethodName)

                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException($"The call back method {callbackContractMethodName} of {callbackResourceContractType.FullName} specified by {nameof(PostMessageActionAttribute.CallbackContractMethodName)} does not exist."));
                    }
                    else

                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException($"There is no call back Method of {callbackResourceContractType.FullName} available to use."));
                }

                if (logicalWaitForReplyAsyncMethod != null)

                    possibleDuplexOperation.Behaviors.Add(new DuplexCallbackMethodBehavior(logicalWaitForReplyAsyncMethod, callbackContractOperation.Name));

                reflectionInfo.duplexCallbackTypesForInputOperations.Add(possibleDuplexOperation, callbackResourceContractType);
        ...
        internal static void EnsureCallbackType(Type callbackType)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(PublicSR.Format(PublicSR.SFxInvalidCallbackContractType, callbackType.Name)));
            }
        }

        // checks a contract for substitutability (in the Liskov Substitution Principle sense), throws on error
        internal static void EnsureSubcontract(PostMessageActionAttribute postActionAttr, MethodInfo contractOp, Type implType)
        {
            if (postActionAttr != null)
            {
                Type callbackType = postActionAttr.CallbackContractType;

                List<MethodInfo> methods = ServiceReflector.GetInheritedMethods<PostMessageActionAttribute>(contractOp, implType);
                for (int i = 0; i < methods.Count; i++)
                {
                    MethodInfo inheritedContractMethod = methods[i];
                    PostMessageActionAttribute inheritedContractAttr = ServiceReflector.GetRequiredSingleAttribute<PostMessageActionAttribute>(inheritedContractMethod);
                    // we must be covariant in our callbacks
                    if (inheritedContractAttr.CallbackContractType != null)
                    {
                        if (callbackType == null)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                                PublicSR.Format(PublicSR.InAContractInheritanceHierarchyIfParentHasCallbackChildMustToo,
                                inheritedContractMethod.Name, inheritedContractAttr.CallbackContractType.Name, contractOp.Name)));
                        }
                        if (!inheritedContractAttr.CallbackContractType.IsAssignableFrom(callbackType))
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                                PublicSR.Format(PublicSR.InAContractInheritanceHierarchyTheServiceContract3_2,
                                inheritedContractMethod.Name, contractOp.Name)));
                        }
                    }
                }
            }
        ...
        private ContractDescription CreateContractDescription(ResourceContractAttribute contractAttr, Type contractType, Type serviceType, out ContractReflectionInfo reflectionInfo, object serviceImplementation)
        {
            ...
            reflectionInfo.duplexCallbackTypesForInputOperations = new Dictionary<OperationDescription, Type>();

            contractDescription.SessionMode = contractAttr.SessionMode;
            contractDescription.CallbackContractType = null;
            contractDescription.ConfigurationName = contractAttr.ConfigurationName ?? contractType.FullName;

            // get inherited operations
            List<Type> types = ServiceReflector.GetInheritedContractTypes<ResourceContractAttribute>(contractType);

            for (int i = 0; i < types.Count; i++)
            {
                Type inheritedContractType = types[i];
                //ResourceInteractionAttribute inheritedContractAttr = ServiceReflector.GetRequiredSingleAttribute<ResourceInteractionAttribute>(inheritedContractType);
                ...
                foreach (OperationDescription op in inheritedContractDescription.Operations)
                {
                    if (!contractDescription.Operations.Contains(op)) // in a diamond hierarchy, ensure we don't add same op twice from two different parents
                    {
                        ...
                        foreach (OperationDescription existingOp in existingOps)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                                    PublicSR.Format(PublicSR.CannotInheritTwoOperationsWithTheSameName3,
            ...
            // this contract 
            CreateOperationDescriptions(contractDescription, reflectionInfo, contractType, false, contractDescription, MessageDirection.Input);

            return contractDescription;
        }

        internal static Attribute GetInheritedSerializerFormatAttribute(Type implType, ICustomAttributeProvider contractAttrProvider)
        {
            if (contractAttrProvider is MethodInfo)
            {
                MethodInfo method = (contractAttrProvider as MethodInfo);

                (String MethodName, Type[] ArgumentTypes) methodSignature = (method.Name, method.GetParameters().Select(p => p.ParameterType).ToArray());

                ICustomAttributeProvider implAttrProvider = implType?.GetMethod(methodSignature.MethodName, methodSignature.ArgumentTypes);

                while (implAttrProvider != null)
                {
                    Attribute formattingAttribute = GetSerializerFormatAttribute(contractAttrProvider, null);

                    if (formattingAttribute != null)

                        return formattingAttribute;

                    implAttrProvider = (implAttrProvider as MethodInfo).DeclaringType.BaseType?.GetMethod(methodSignature.MethodName, methodSignature.ArgumentTypes);
                }
            }
            else

                throw new NotImplementedException($"{nameof(ResourceTypeLoader)}::{nameof(GetInheritedSerializerFormatAttribute)} for ${contractAttrProvider.GetType()}");

            return GetSerializerFormatAttribute(contractAttrProvider, null);
        }

        internal static Attribute GetSerializerFormatAttribute(ICustomAttributeProvider attrProvider, Attribute defaultFormatAttribute)
        {
            if (attrProvider != null)
            {
                if (attrProvider.IsDefined(typeof(ResourceTransferFormatSerializerAttribute), false))
                {
                    return ServiceReflector.GetSingleAttribute<ResourceTransferFormatSerializerAttribute>(attrProvider, s_formatterAttributes);
                }
                if (attrProvider.IsDefined(typeof(XmlAttributeOverridesSerializerFormatAttribute), false))
                {
                    return ServiceReflector.GetSingleAttribute<XmlAttributeOverridesSerializerFormatAttribute>(attrProvider, s_formatterAttributes);
                }
        ...
        private IEnumerable<OperationDescription> CreateOperationDescriptions(
            ContractDescription contractDescription, 
            ...
            ContractDescription declaringContract)
        {
            IEnumerable<ResourceEndpointAttribute> resourceMethodAttrs = 
                ServiceReflector.GetOperationContractAttributes<ResourceEndpointAttribute>(methodInfo, out _);

            return resourceMethodAttrs.Aggregate(new List<OperationDescription>(), (ops, attr) =>
            {
                ops.Add(
                    CreateOperationDescription(contractDescription, attr, methodInfo, isCallbackMethod, direction, reflectionInfo, declaringContract)
                );
        ...
        private OperationDescription CreateOperationDescription(ContractDescription contractDescription, ResourceEndpointAttribute opAttr, MethodInfo methodInfo, bool isCallbackMethod, MessageDirection direction, ContractReflectionInfo reflectionInfo, ContractDescription declaringContract)
        {
            if (opAttr == null)
            {
                return null;
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                    PublicSR.Format(PublicSR.EndMethodsCannotBeDecoratedWithOperationContractAttribute,
                                 methodInfo.Name, reflectionInfo.iface)));
            }

            Type taskTResult;
            bool isTask = ServiceReflector.IsTask(methodInfo, out taskTResult);
            bool isAsync = isTask ? false : opAttr.IsBegin(methodInfo);
            ...
            for (int i = 0; i < operations.Count; i++)
            {
                ...
                if (existingOp.Messages[0].Direction == direction)
                {
                    ...
                    if (existingOp.TaskMethod != null && isTask)
                    {
                        ...
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.CannotHaveTwoOperationsWithTheSameName3, method1Name, method2Name, reflectionInfo.iface)));
                    }
            ...
            operationDescription.Behaviors.Add(opAttr);
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.OneWayAndFaultsIncompatible2, methodInfo.DeclaringType.FullName, operationName.EncodedName)));
            }
            ...
            MessageActionAttribute messageActionAttr = opAttr as MessageActionAttribute;

            string requestAction = null;
            string responseAction = null;

            if (messageActionAttr != null)
            {
                requestAction = NamingHelper.GetMessageAction(
                    contractQname,
                    operationDescription.CodeName,
                    messageActionAttr.Action,
                    false);

                responseAction = NamingHelper.GetMessageAction(
                    contractQname,
                    operationDescription.CodeName,
                    messageActionAttr.ReplyAction,
                    true);
            }
            else
            {
                // actions for non-action oriented resources are generally ignored, but we supply them here because the 
                // validation routines were origonally written for action oriented servcices ...
                requestAction = NamingHelper.GetMessageAction(
                    contractQname,
                    operationDescription.CodeName,
                    null,
            ...
            if (isCallbackMethod)
            {
                if (opAttr.IsOneWay())
                {
                    ...
                    {
                        MethodInfo outputMethod = ServiceReflector.GetEndMethod<ResourceActionAttribute>(a => a.AsyncPattern(), methodInfo);
                ...
                {
                    if ((!isTask && methodInfo.ReturnType != ServiceReflector.VoidType) || (isTask && taskTResult != ServiceReflector.VoidType) ||
                        ServiceReflector.HasOutputParameters(methodInfo, isAsync))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.ServiceOperationsMarkedWithIsOneWayTrueMust0));
                    }

                    if (opAttr.ReplyAction() != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.OneWayOperationShouldNotSpecifyAReplyAction1, operationName)));
                    }
                }
            }
            else
            {
                ...
                {
                    outputMethod = ServiceReflector.GetEndMethod<ResourceActionAttribute>(a => a.AsyncPattern(), methodInfo);
                    operationDescription.EndMethod = outputMethod;
                    operationDescription.BeginMethod = methodInfo;
                }
        ...
        private void CheckDuplicateFaultContract(FaultDescriptionCollection faultDescriptionCollection, FaultDescription fault, string operationName)
        {
            foreach (FaultDescription existingFault in faultDescriptionCollection)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        PublicSR.Format(PublicSR.SFxFaultContractDuplicateDetailType, operationName, fault.DetailType)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        PublicSR.Format(PublicSR.SFxFaultContractDuplicateElement, operationName, fault.ElementName, fault.Namespace)));
                }
            }
        }
        ...
        private MessageDescription CreateMessageDescription(ResourceEndpointAttribute resourceOperationdAttribute, MethodInfo methodInfo, bool isAsync, bool isTask, Type taskTResult, XmlName returnValueName, string defaultNS, string action, XmlName wrapperName, string wrapperNamespace, MessageDirection direction)
        {
            ...
            if (returnValueName == null)
            {
                ...
                if (parameters.Length == 1 && parameters[0].ParameterType.IsDefined(typeof(MessageContractAttribute), false))
                {
                    messageDescription =
                        CreateTypedMessageDescription(
                            ...
                            resourceOperationdAttribute.GetRequestContractAttribute(wrapperName.EncodedName, wrapperNamespace),
                            null,
                            null,
                            defaultNS,
                            action,
                ...
                {
                    messageDescription =
                        CreateParameterMessageDescription(
                            parameters,
                            null,
                            null,
                            null,
            ...
            else
            {
                ...
                if (parameters.Length == 0 && responseType.IsDefined(typeof(MessageContractAttribute), false))
                {
                    messageDescription =
                        CreateTypedMessageDescription(
                            responseType,
                            resourceOperationdAttribute.GetResponseContractAttribute(wrapperName.EncodedName, wrapperNamespace),
            ...
            for (int i = 0; i < messageDescription.Headers.Count; i++)
            {
                ...
                if (header.IsUnknownHeaderCollection)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxMultipleUnknownHeaders, methodInfo, methodInfo.DeclaringType)));
                    }
                    else
                    {
                                                  ...
                                                  MessageDirection direction)
        {
            foreach (ParameterInfo param in parameters)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxInvalidMessageContractSignature, methodName)));
                }
            }
            ...
            for (int index = 0; index < parameters.Length; index++)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidMessageContractException(PublicSR.Format(PublicSR.SFxDuplicateMessageParts, partDescription.Name, partDescription.Namespace)));
                }
        ...
        internal MessageDescription CreateTypedMessageDescription(
            ...
            MessageDirection direction)
        {
            ...
            for (Type baseType = typedMessageType; baseType != null && baseType != typeof(object) && baseType != typeof(ValueType); baseType = baseType.BaseType)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxMessageContractBaseTypeNotValid, baseType, typedMessageType)));
                }
                                                                    ...
                                                                    int serializationPosition)
        {
            ...
            if (headerAttr is MessageHeaderArrayAttribute)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxInvalidMessageHeaderArrayType, defaultName)));
                }
            ...
            if (headerDescription.TypedHeader)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(PublicSR.Format(PublicSR.SFxStaticMessageHeaderPropertiesNotAllowed, defaultName)));
                }
            }
            else
            {
        ...
        private class SyncAsyncOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.SyncAsyncMatchConsistency_Parameters5,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.SyncAsyncMatchConsistency_ReturnType5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
                                                                   _asyncOperation.BeginMethod.Name,
                                                                   _asyncOperation.EndMethod.Name,
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.SyncAsyncMatchConsistency_Property6,
                                                                       _syncOperation.SyncMethod.Name,
                                                                       _syncOperation.SyncMethod.DeclaringType,
                                                                       _asyncOperation.BeginMethod.Name,
                                                                       _asyncOperation.EndMethod.Name,
        ...
        private class SyncTaskOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.SyncTaskMatchConsistency_Parameters5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.SyncTaskMatchConsistency_ReturnType5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
                                                                   _taskOperation.TaskMethod.Name,
                                                                   _syncOperation.Name)));
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.SyncTaskMatchConsistency_Property6,
                                                                       _syncOperation.SyncMethod.Name,
                                                                       _syncOperation.SyncMethod.DeclaringType,
                                                                       _taskOperation.TaskMethod.Name,
                                                                       _syncOperation.Name,
        ...
        private class TaskAsyncOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.TaskAsyncMatchConsistency_Parameters5,
                                                                   _taskOperation.TaskMethod.Name,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(PublicSR.Format(PublicSR.TaskAsyncMatchConsistency_ReturnType5,
                                                                   _taskOperation.TaskMethod.Name,
                                                                   _taskOperation.TaskMethod.DeclaringType,
                                                                   _asyncOperation.BeginMethod.Name,
                                                                   _asyncOperation.EndMethod.Name,
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(PublicSR.Format(PublicSR.TaskAsyncMatchConsistency_Property6,
                                                                       _taskOperation.TaskMethod.Name,
                                                                       _taskOperation.TaskMethod.DeclaringType,
                                                                       _asyncOperation.BeginMethod.Name,
                                                                       _asyncOperation.EndMethod.Name,
        ...
        {
            internal Type iface;
            internal Dictionary<OperationDescription, Type> duplexCallbackTypesForInputOperations;
        }
